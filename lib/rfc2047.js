...
// Characters with special meaning you might want to escape in some contexts,
// like the name part of an e-mail address, eg.
// "Jon (Marketing) <jon@mark.test>"
const headerSpecialChars = /[()<>@,;:"\/\[\]?.=]/;
...
rfc2047.encode = (text, escapeSpecialChars = false) => {
  ...
  if (tokens) {
    for (let i = 0; i < tokens.length; i += 1) {
      let token = tokens[i];
      if (
        (escapeSpecialChars && headerSpecialChars.test(token)) ||
        unsafeTokenRegExp.test(token) ||
        (token && nextTokenMustBeEncoded)
      ) {
